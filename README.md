...
- **补齐 EpochHead 套装效果行**
  - 将完整 Tooltip 中的 `(2) Set:` / `(4) Set:` 等套装效果纳入绿字生成范围
  - 绿字精确覆盖从 2,848 条扩展到 3,100 条，剩余人工审校缺口为 0
  - 补充 96 条高频套装效果翻译，覆盖职业套装、PvP 套装和特殊触发效果
- **更新抓库与生成链路**
  - EpochHead 抓取脚本会把套装效果识别为效果行，后续同步不再漏掉这类文本
  - 同步刷新 `EpochItemData.lua` 与 `TooltipLineData.lua`，减少游戏内英文绿字残留
